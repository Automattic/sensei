...
- **`Resolves #`** — if the user gave an issue number (or it's clearly in the
  branch name or commits), fill it in as `Resolves #<n>`. If there's no issue,
  **remove this line entirely** — a bare `Resolves #` is noise. Never guess a
  number.
