...
- **wp-cli goes through `scripts/triage-wp`, never `make wp`.** Permitted verbs: `post`, `term`, `user`, `option get`, `plugin list`, `theme list`, `theme activate`, `action-scheduler`, `sensei db seed`. Call it as `scripts/triage-wp <verb> <args>` — no `CMD="…"` wrapper. The script refuses any other verb, and any argument containing characters outside `A-Za-z0-9_./=:@,+-` and space, so quote your values in the outer shell rather than passing quote characters through. `wp eval`, `wp shell`, `wp db` and `wp plugin install` are unavailable: arbitrary code execution with network access would bypass the browser confinement. `make wp` itself is **not** allowlisted — the Makefile interpolates the command into a shell recipe, so a glob over it can't constrain what runs.
