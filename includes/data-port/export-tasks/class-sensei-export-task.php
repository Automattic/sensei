...
abstract class Sensei_Export_Task extends Sensei_Data_Port_Task implements Sensei_Data_Port_Task_Interface {
	...
	public function __construct( Sensei_Data_Port_Job $job ) {
		...
		$resolved_ids = $this->get_resolved_ids();
		if ( ! empty( $resolved_ids ) ) {
			$query_args['post__in'] = $resolved_ids;
	...
	/**
	 * Get the resolved post IDs to restrict this task to.
	 *
	 * Returns an empty array when no selection has been made (export all).
	 *
	 * @since $$next-version$$
	 *
	 * @return int[]
	 */
	private function get_resolved_ids() {
		$job = $this->get_job();
		...
		return $job->get_resolved_ids( $this->get_content_type() );
	}
