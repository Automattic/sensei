...
class Sensei_Home_Task_Publish_First_Course implements Sensei_Home_Task {
	...
	protected function load_result() {
		...
		if ( false === $result ) {
			...
			$result = $wpdb->get_row( $wpdb->prepare( "SELECT ID, post_status='publish' as published FROM {$wpdb->posts} WHERE post_type='course' AND post_name NOT LIKE %s ORDER BY published DESC, ID DESC LIMIT 1", "{$prefix}%" ) );
