...
class Sensei_Main {
	...
	public function check_user_permissions($page = '') {
		...
		if (empty($current_user->caps) && Sensei()->settings->get('access_permission')
			...
			$this->permissions_message['message'] = __('You must be <a href="' . wp_login_url(get_permalink()) . '" title="Login">logged in</a> to view this ' . get_post_type());

			if (get_option('users_can_register')):
				$this->permissions_message['message'] .= ". <a href=\"" . bloginfo('wpurl') . "/wp-login.php?action=register\">Register here</a>, if you do not have an account.";
