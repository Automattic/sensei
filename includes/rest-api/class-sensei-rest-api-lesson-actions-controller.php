...
class Sensei_REST_API_Lesson_Actions_Controller extends \WP_REST_Controller {
	...
	public function register_routes() {
		register_rest_route(
			...
			[
				...
				'callback'            => [ $this, 'attach_lessons' ],
				'permission_callback' => [ $this, 'attach_lessons_permissions_check' ],
				'args'                => [
					'lesson_ids' => [
	...
	/**
	 * Attach lessons to a course. This duplicates lessons if they are already related to a course.
	 *
	 * @since $$next-version$$
	 *
	 ...
	 */
	public function attach_lessons( $request ) {
	...
	/**
	 * Check if the current user can attach lessons to a course.
	 *
	 * @since $$next-version$$
	 *
	 * @param WP_REST_Request $request Full details about the request.
	 ...
	 */
	public function attach_lessons_permissions_check( $request ): bool {
		$lesson_ids = $request->get_param( 'lesson_ids' );
		$course_id  = $request->get_param( 'course_id' );
