...
//			'fields' => 'all_with_meta'
		...
		// Get Users
		$users = $this->user_query_results( apply_filters( 'sensei_analysis_filter_users', $args_array ) );
...
//		$posts_array = $woothemes_sensei->post_types->course->course_query( -1, 'usercourses' );
		...
		$posts_array = get_posts( apply_filters( 'sensei_analysis_filter_courses', $post_args ) );
		// MAIN LOOP
		foreach ($posts_array as $course_item) {
			...
			if ( 0 < intval( $title_keyword_count ) ) {
				// Course Completions
				$course_completions = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_analysis_activity_courses_ended', array( 'post_id' => $course_item->ID, 'type' => 'sensei_course_end' ) ) );
				// Course Students
				$course_students = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_analysis_activity_courses_started', array( 'post_id' => $course_item->ID, 'type' => 'sensei_course_start' ) ) );
				// Course Lessons
	...
	public function overview_lessons( $args_array ) {
		...
		$posts_array = get_posts( apply_filters( 'sensei_analysis_filter_lessons', $post_args ) );
		// MAIN LOOP
		foreach ($posts_array as $lesson_item) {
			...
			if ( 0 < intval( $title_keyword_count ) ) {
				...
				// Lesson Students
				$lesson_students = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_analysis_activity_lessons_started', array( 'post_id' => $lesson_item->ID, 'type' => 'sensei_lesson_start' ) ) );
				// Lesson Completions
				$lesson_completions = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_analysis_activity_lessons_ended', array( 'post_id' => $lesson_item->ID, 'type' => 'sensei_lesson_end' ) ) );
				// Lesson Grades
				$lesson_grades = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_analysis_activity_lesson_grades', array( 'post_id' => $lesson_quiz_id, 'type' => 'sensei_quiz_grade' ) ), true );
				$total_grade_count = 0;
	...
	public function load_stats() {
		...
		$this->user_count = apply_filters( 'sensei_analysis_total_users', $user_count['total_users'], $user_count );
		...
		$total_quiz_grades = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_activity_total_quiz_grades', array( 'type' => 'sensei_quiz_grade' ) ), true );
		$total_grade_count = 0;
		$total_grade_total = 0.00;
		// Calculate the average quiz grade
		...
		$this->total_courses_started = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_activity_total_courses_started', array( 'type' => 'sensei_course_start' ) ) );
		$this->total_courses_ended = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_activity_total_courses_ended', array( 'type' => 'sensei_course_end' ) ) );
