		    		...
		    		foreach ($course_lessons as $lesson_item){
						...
						if ( '' != $user_lesson_end ) {
		                    ...
		                    if ( 'passed' == $course_completion ) {
		                        ...
		                        // Get Quiz ID
		                        $lesson_quiz_id = $this->post_types->lesson->lesson_quizzes( $lesson_item->ID );
		                        if ( $lesson_quiz_id ) {
		                            // Quiz Grade
				...
				foreach ($courses as $course_item){
	    			...
	    			foreach ($course_lessons as $lesson_item){
	        			...
	        			if ( $lesson_quiz_id )  {
							...
							$dataset_changes = WooThemes_Sensei_Utils::sensei_delete_activities( array( 'post_id' => intval( $lesson_quiz_id ), 'user_id' => $user_id, 'type' => 'sensei_quiz_grade' ) );
							$dataset_changes = WooThemes_Sensei_Utils::sensei_delete_activities( array( 'post_id' => intval( $lesson_quiz_id ), 'user_id' => $user_id, 'type' => 'sensei_quiz_asked' ) );
							$delete_answers = WooThemes_Sensei_Utils::sensei_delete_quiz_answers( intval( $lesson_quiz_id ), $user_id );
	    				} // End If Statement
	    			} // End For Loop
				} // End For Loop
			} // End For Loop
