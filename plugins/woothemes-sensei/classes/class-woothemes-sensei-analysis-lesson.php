...
class WooThemes_Sensei_Analysis_Lesson_List_Table extends WooThemes_Sensei_List_Table {
	...
	public function __construct ( $lesson_id = 0 ) {
		...
		if ( isset( $this->lesson_id ) && 0 < intval( $this->lesson_id ) ) {
			$this->user_ids = WooThemes_Sensei_Utils::sensei_activity_ids( apply_filters( 'sensei_analysis_activity_lessons_started', array( 'post_id' => intval( $this->lesson_id ), 'type' => 'sensei_lesson_start', 'field' => 'user_id' ) ) );
		} // End If Statement
...
//			'fields' => 'all_with_meta'
		...
		$output_counter = 0;
		// Get Quiz ID
		$lesson_quiz_id = $woothemes_sensei->post_types->lesson->lesson_quizzes( $this->lesson_id );
