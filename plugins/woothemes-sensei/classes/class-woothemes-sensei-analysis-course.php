	...
	public function __construct ( $course_id = 0, $user_id = 0 ) {
		...
		if ( isset( $_GET['user'] ) && -1 == intval( $_GET['user'] ) && isset( $_GET['course_id'] ) && 0 < intval( $_GET['course_id'] ) ) {
			...
			if ( isset( $this->course_id ) && 0 < intval( $this->course_id ) ) {
				$this->user_ids = WooThemes_Sensei_Utils::sensei_activity_ids( apply_filters( 'sensei_analysis_activity_courses_started', array( 'post_id' => intval( $this->course_id ), 'type' => 'sensei_course_start', 'field' => 'user_id' ) ) );
			} // End If Statement
			// Default Columns
...
//			$posts_array = $woothemes_sensei->post_types->course->course_lessons( $this->course_id );
			...
			foreach ($posts_array as $lesson_item) {
				...
				if ( 0 < intval( $title_keyword_count ) ) {
			    	...
			    	$lesson_start_date = $lesson_students = WooThemes_Sensei_Utils::sensei_activity_ids( apply_filters( 'sensei_analysis_activity_lessons_started', array( 'post_id' => $lesson_item->ID, 'type' => 'sensei_lesson_start', 'field' => 'user_id' ) ) );
			    	// Check if Lesson is complete
			    	$user_lesson_end = WooThemes_Sensei_Utils::sensei_activity_ids( apply_filters( 'sensei_analysis_activity_lessons_ended', array( 'post_id' => $lesson_item->ID, 'type' => 'sensei_lesson_end', 'field' => 'user_id' ) ) );
					...
					if ( 0 < count( $lesson_start_date ) ) {
						// Quiz Grade
						$quiz_grade =  WooThemes_Sensei_Utils::sensei_get_activity_value( apply_filters( 'sensei_analysis_activity_lesson_grades', array( 'post_id' => $lesson_quiz_id, 'user_id' => $this->user_id, 'type' => 'sensei_quiz_grade', 'field' => 'comment_content' ) ) );
			    	...
			    	if ( 0 == $this->user_id ) {
			    		...
			    		$total_quiz_grades = WooThemes_Sensei_Utils::sensei_check_for_activity( apply_filters( 'sensei_analysis_activity_lesson_grades', array( 'type' => 'sensei_quiz_grade' ) ), true );
