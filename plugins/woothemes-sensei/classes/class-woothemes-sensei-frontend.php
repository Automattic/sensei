...
class WooThemes_Sensei_Frontend {
	...
	public function sensei_complete_lesson() {
		...
		if ( isset( $_POST['quiz_complete'] ) && wp_verify_nonce( $_POST[ 'woothemes_sensei_complete_lesson_noonce' ], 'woothemes_sensei_complete_lesson_noonce' ) ) {
			// Lesson Quiz Meta
			$lesson_quiz_id = $woothemes_sensei->frontend->lesson->lesson_quizzes( $post->ID );
		    	...
		    	case apply_filters( 'sensei_mark_as_complete_text', __( 'Mark as Complete', 'woothemes-sensei' ) ):
					...
					if ( $activity_logged ) {
		    			...
		    			foreach ($course_lessons as $lesson_item){
							...
							if ( $activity_logged ) {
								// Lesson Quiz Meta
		        				$lesson_quiz_id = $this->lesson->lesson_quizzes( $lesson_item->ID );
		        				if ( $lesson_quiz_id )  {
									...
									$quiz_passmark = absint( get_post_meta( $lesson_quiz_id, '_quiz_passmark', true ) );
									do_action( 'sensei_user_quiz_grade', $current_user->ID, $lesson_quiz_id, 100, $quiz_passmark );
								} // End If Statement
							} // End If Statement
						} // End For Loop
		    	...
		    	case apply_filters( 'sensei_delete_course_text', __( 'Delete Course', 'woothemes-sensei' ) ):
	    			...
	    			foreach ($course_lessons as $lesson_item){
	        			...
	        			if ( $lesson_quiz_id )  {
							...
							$delete_answers = WooThemes_Sensei_Utils::sensei_delete_quiz_answers( $lesson_quiz_id, $current_user->ID );
							// Check for quiz grade
							$dataset_changes = WooThemes_Sensei_Utils::sensei_delete_activities( array( 'post_id' => $lesson_quiz_id, 'user_id' => $current_user->ID, 'type' => 'sensei_quiz_grade' ) );
	    				} // End If Statement
	    			} // End For Loop
	    			// Run any action on course reset
	...
	public function sensei_lesson_quiz_meta( $post_id = 0, $user_id = 0 ) {
		...
		if( intval( $lesson_prerequisite ) > 0 ) {
			...
			// Lesson Quiz Meta
			$prerequisite_quiz_id = $woothemes_sensei->frontend->lesson->lesson_quizzes( $lesson_prerequisite );
		...
		?><header><?php
		if ( $quiz_id && is_user_logged_in() && sensei_has_user_started_course( $lesson_course_id, $user_id ) ) { ?>
            <?php $no_quiz_count = 0; ?>
        	<?php
					...
					if( $courses && count( $courses ) > 0 ) {
						foreach( $courses as $course ) {
			    			...
			    			foreach ($course_lessons as $lesson_item){
			        			...
			        			$lesson_quiz_id = $this->lesson->lesson_quizzes( $lesson_item->ID );
			        			if ( $lesson_quiz_id )  {
									$delete_answers = WooThemes_Sensei_Utils::sensei_delete_quiz_answers( $lesson_quiz_id, $user_id );
									$dataset_changes = WooThemes_Sensei_Utils::sensei_delete_activities( array( 'post_id' => $lesson_quiz_id, 'user_id' => $user_id, 'type' => 'sensei_quiz_grade' ) );
			    				} // End If Statement
			    			} // End For Loop
						} // End For Loop
					} // End If Statement
