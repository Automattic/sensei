...
if ( ! $paged || $paged < 2 ) {
	...
	// Restrict the Courses shown to those that are in the current Programme
	$courseIDs = $imp->get_courses_by_programme( $imp->programme, false, 'ids' );
	if ( count($courseIDs) ) {
		foreach( $courseIDs as $courseID ) {
			// This is a "private" course, don't show...
			$private = get_post_meta( $courseID, 'course_private', $single = true );
			if ( !empty($private) ) {
				if ( null == $user_course_ids ) {
					$user_course_ids = $imp->get_user_course_ids();
				}
				// ...unless the User is "on" it
				if ( in_array($courseID, $user_course_ids) ) {
					$course_includes[] = $courseID;
				}
			}
			// Not private
			else {
		...
		$course_includes = array(1); // Technically nothing to show
	}
	// NOTE: Using $course_includes ('includes') means the get_posts() call ignores the $amount ('posts_per_page') value
...
//    		} // End If Statement ?>
	...
	<?php } // End If Statement
		...
		echo '<p>' . __('This programme currently has no courses associated with it.', 'imperial') . '</p>';
	}
} else {
