...
<li class="multiple-choice">
    <span><?php echo esc_html( $question_text ); ?> <span>[<?php echo $question_grade; ?>]</span></span>
    ...
    <?php foreach( $answers_sorted as $id => $question ) {
        ...
        if( isset( $user_correct ) ) {
            ...
            if ( is_array($question_right_answer) && in_array($question, $question_right_answer) ) {
                $answer_class .= ' right_answer';
            }
            elseif( !is_array($question_right_answer) && $question_right_answer == $question ) {
                $answer_class .= ' right_answer';
            }
        }

        if ( isset( $user_quizzes[ $question_id ] ) && 0 < count( $user_quizzes[ $question_id ] ) ) {
            if ( is_array( $user_quizzes[ $question_id ] ) && in_array( $question, $user_quizzes[ $question_id ] ) ) {
        ...
        <li class="<?php esc_attr_e( $answer_class ); ?>">
            <input type="<?php echo $answer_type; ?>" id="<?php echo esc_attr( 'question_' . $question_id ) . '-option-' . $count; ?>" name="<?php echo esc_attr( 'sensei_question[' . $question_id . ']' ); ?>[]" value="<?php echo esc_attr( $question ); ?>" <?php echo $checked; ?><?php if ( !is_user_logged_in() ) { echo ' disabled'; } ?>>&nbsp;
            <label for="<?php echo esc_attr( 'question_' . $question_id ) . '-option-' . $count; ?>"><?php echo apply_filters( 'sensei_answer_text', $question ); ?></label>
        </li>
    <?php } // End For Loop ?>
    </ul>
    <?php if( $answer_notes ) { ?>
