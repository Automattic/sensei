 ...
 * @param {string}   props.attributes.lessonSubheaderFontSize The lesson subheader font size.
 * @param {Function} props.setAttributes                      Callback method to set the attributes for module block.
 */
const ModuleSettings = ( {
	...
	customSlug,
	setCustomSlug,
	attributes: { lessonSubheaderFontSize },
	setAttributes,
} ) => {
	...
	return (
		<InspectorControls>
			...
			<PanelBody
				title={ __( 'Lesson Subheading Font Size', 'sensei-lms' ) }
				initialOpen={ false }
			>
				<FontSizePicker
					fontSizes={ fontSizes }
					value={ lessonSubheaderFontSize }
					onChange={ ( value ) => {
						setAttributes( {
							lessonSubheaderFontSize: value,
						} );
					} }
				/>
			</PanelBody>
