...
class StudentsModalFragment {
	constructor( locator ) {
		this.base = locator;
		this.addToCourseButton = this.base.locator( 'button:has-text("Add to Course")' );

		this.removeFromCourseButton = this.base.locator( 'button:has-text("Remove from Course")' );
	}
...
class StudentsPage {
	constructor( page ) {
		...
		this.enrolledCoursesColumn = page.locator( '[data-colname="Enrolled Courses"]' );
	}

	// Migrate to role selector as soon it is available.
	async getRowByStudent( studentName ) {
		return this.page.locator( `tr:has([data-user-name="${ studentName }"])` );
	}
	async goTo() {
	...
	async openStudentAction( studentName, action ) {
		await this.page.locator( `[data-user-name="${ studentName }"]` ).locator( `button[aria-label="Select an action"]` ).click();
