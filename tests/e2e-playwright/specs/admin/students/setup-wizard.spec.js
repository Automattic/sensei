...
test.describe.serial( 'Setup Wizard @setup', () => {
	...
	test.describe.serial( 'Welcome step', () => {
		...
		test( 'displays usage tracking modal when clicking continue', async () => {
			...
			await expect( page.locator( 'text=Build a Better Sensei LMS' ) ).toHaveCount( 1 );
		} );

		test( 'marks welcome step done and goes to purpose step', async () => {
			await pluginsPage.closeUserTrackingModal();
			await expect( page.locator( 'text=What is your primary purpose for offering online courses?' ) ).toHaveCount( 1 );
		} );
	} );
	...
	test.describe.serial( 'Purpose step', () => {
		...
		test( 'marks purpose step done and goes to features step', async () => {
			await expect( page.locator( 'text=Enhance your online courses with these optional features.' ) ).toHaveCount( 1 );
		} );
	} );
