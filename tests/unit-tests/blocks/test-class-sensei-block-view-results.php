...
<!-- /wp:sensei-lms/button-view-results -->';
	...
	public function testRender_CourseCompleted_ReturnsModifiedBlockContent() {
		...
		$this->assertMatchesRegularExpression(
			'|<form method="get" action="' . preg_quote( home_url( '/' ), '|' ) . '\?page_id=' . $page_id . '&#038;course_id=' . $this->course->ID . '".*><input type="hidden" name="page_id" value="' . $page_id . '"><input type="hidden" name="course_id" value="' . $this->course->ID . '">|',
			$result
		);
	}
