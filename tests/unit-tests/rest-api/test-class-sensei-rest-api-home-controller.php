...
class Sensei_REST_API_Home_Controller_Test extends WP_Test_REST_TestCase {
	...
	public function testHomeDataReturnsQuickLinksGeneratedByProviderAndMappedByMapper() {
		...
		$promo_provider_stub = $this->createMock( Sensei_Home_Sensei_Pro_Promo_Provider::class );
	...
	/**
	 * Check that Home Data contains the boolean for the show sensei pro promo section as returned by the provider.
	 */
	public function testHomeDataReturnsSenseiProPromoBooleanAsReturnedByProvider() {
		// Stubs
		$quick_links_provider_stub = $this->createMock( Sensei_Home_Quick_Links_Provider::class );
		$mapper_stub               = $this->createMock( Sensei_REST_API_Home_Controller_Mapper::class );
		$help_provider_stub        = $this->createMock( Sensei_Home_Help_Provider::class );
		// Mock provider
		$mocked_boolean      = false;
		$promo_provider_mock = $this->createMock( Sensei_Home_Sensei_Pro_Promo_Provider::class );
		$promo_provider_mock->expects( $this->once() )
			->method( 'get' )
			->willReturn( $mocked_boolean );
		...
		// Assert 'help' is returned as received from the mapper.
		$this->assertArrayHasKey( 'show_sensei_pro_promo', $result );
		$this->assertSame( $mocked_boolean, $result['show_sensei_pro_promo'] );
	}
}
