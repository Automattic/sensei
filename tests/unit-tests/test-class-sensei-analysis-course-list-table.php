...
class Sensei_Analysis_Course_List_Table_Test extends WP_UnitTestCase {
	...
	public function testTableFooter_WhenCalled_DisplayTheExportButtonWithCorrectArgs() {
		...
		/* Assert. */
		$expected = sprintf(
			'<a class="button button-primary" href="' . admin_url( 'admin.php' ) . '?page=sensei_reports&#038;course_id=%d&#038;view=lesson&#038;sensei_report_download=%s-%s-lessons-overview&#038;start_date=2022-03-01&#038;end_date=2022-03-02&#038;s=course&#038;user_id=%d&#038;_sdl_nonce=%s">Export all rows (CSV)</a>',
			$course->ID,
			$user->user_nicename,
			$course->post_name,
			$user->ID,
