...
class Sensei_Class_Course_Test extends WP_UnitTestCase {
	...
	public function testCourseArchiveOrderSetOrderByDoesNotAddParametersOutsideOfArchivePage() {
		...
		// First consecutive response is *not* for the course archive page, second response is for false positive cases, like 'wp-admin -> Sensei LMS -> Courses' and queries generated from shortcode
		...
		$wp_query->expects( $this->once() )
			...
			->willReturn( false );
		//First call to check the case when the query is not from archive page
		Sensei_Course::course_archive_set_order_by( $wp_query );
		//Second call to check when the query is from any source other than archive, like a shortcode but is_post_type_archive returns true
		Sensei_Course::course_archive_set_order_by( $wp_query );
	}
